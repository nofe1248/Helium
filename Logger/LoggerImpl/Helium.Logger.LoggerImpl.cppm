    ...
    public:
        ...
        auto log(LogLevel log_level, std::string_view fmt_str, Args &&...fmt_args) const -> void {
            this->logger_ptr_->log(static_cast<spdlog::level::level_enum>(log_level),
                                   std::format("[{}/{}] {}", this->name_, this->thread_,
                                               std::vformat(fmt_str, std::make_format_args(fmt_args...))));
        }
        ...
        auto logw(LogLevel log_level, std::wstring_view fmt_str, Args &&...fmt_args) const -> void {
            this->logger_ptr_->log(static_cast<spdlog::level::level_enum>(log_level),
                                   std::vformat(fmt_str, std::make_wformat_args(std::forward<Args>(fmt_args)...)));
        }
