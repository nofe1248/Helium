...
export namespace helium::base {
    ...
    public:
        ...
        [[nodiscard]] auto lock() const noexcept { return RefType::_try_lock(this->weak_ptr_); }
        [[nodiscard]] auto useCount() const noexcept -> long { return this->weak_ptr_.use_count(); }
    };
    ...
    public:
        ...
        auto operator->() const noexcept -> const StateType * { return this->state_.get(); }

        operator StateType const &() const noexcept { return *this->state_; }

        [[nodiscard]] auto clone() const noexcept -> RefType { return RefType::_from_ptr(this->state_).clone(); }

        auto unshare() noexcept -> void { this->state_ = this->clone().state_; }

        [[nodiscard]] auto weakRef() const noexcept -> WeakRef<RefType const> {
            return RefType::_from_ptr(this->state_);
        }

        operator WeakRef<RefType const>() const noexcept { return this->weakRef(); }
    ...
    private:
        ...
        auto _as_derived() noexcept -> auto & { return static_cast<Derived &>(*this); }
        auto _as_derived() const noexcept -> auto & { return static_cast<Derived const &>(*this); }
    ...
    public:
        ...
        operator HeliumSharedObjectBase &() const noexcept { return *this->state_; }
        ...
        [[nodiscard]] static auto cloneFrom(StateType &&state) noexcept(noexcept(StateType{std::move(state)}))
                -> Derived {
        ...
        auto unshare() noexcept -> void { this->_as_derived() = this->clone(); }
