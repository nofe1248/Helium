...
public:
    ...
    {
        return "VelocityServerOutputParser";
    }
    ...
    {
        RE2 player_join_regex{R"(\[connected player] ([^ ]+) \(\/([^ ]+):(\d+)\) has connected)"};
    ...
    {
        RE2 player_join_regex{R"(\[connected player] ([^ ]+) \(\/([^ ]+):(\d+)\) has disconnected)"};
        std::string player_name;

        if (RE2::FullMatch(preprocessed_output, player_join_regex, &player_name))
        {
    ...
    {
        RE2 started_regex{R"(Done \([0-9.]+s\)!)"};
    ...
    {
        return preprocessed_output == "Shutting down the proxy...";
    }
};
