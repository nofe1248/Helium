...
export module Helium.Server.ServerOutputParser.BukkitLegacy;
...
class BukkitLegacyServerOutputParser final : public base::HeliumObject
{
public:
    constexpr auto getParserName(this auto &&self) noexcept -> std::string
    {
        return "BukkitLegacyServerOutputParser";
    }
    ...
    {
        RE2 player_message_regex{R"((\[Not Secure] )?(\[[a-z0-9_:]+])?<([^>]+)> (.*))"};
        std::string no_secure, player_name, message;

        if (RE2::FullMatch(preprocessed_output, player_message_regex, &no_secure, static_cast<void *>(nullptr), &player_name, &message))
        {
            if (verifyPlayerName(player_name))
            {
                return PlayerMessage{.is_secure = no_secure.empty(), .player_name = player_name, .player_message = message};
            }
        }
