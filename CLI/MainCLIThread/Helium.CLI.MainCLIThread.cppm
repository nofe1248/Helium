...
export module Helium.CLI.MainCLIThread;
...
auto completionHook(std::string const &context, int &context_len) -> repl::Replxx::completions_t
{
    ...
    if (context_len <= 1)
    {
        suggestions = commands::CommandDispatcher::getInstance().getSuggestions(context, 0.0f, true);
    }
    else
    {
        suggestions = commands::CommandDispatcher::getInstance().getSuggestions(context, 50.0f);
    }
...
auto hintHook(std::string const &context, int &context_len, repl::Replxx::Color &color) -> repl::Replxx::hints_t
{
    ...
    if (context_len <= 1)
    {
        hints = commands::CommandDispatcher::getInstance().getSuggestions(context, 0.0f, true);
    }
    else
    {
        hints = commands::CommandDispatcher::getInstance().getSuggestions(context, 60.0f);
    }
    if (hints.size() == 1)
    {
...
class MainCLIThread final : public base::HeliumObject
{
private:
    std::jthread thread_{};

    auto mainCLILoop(std::stop_token st)
    {
        ...
        while (not st.stop_requested())
        {
            ...
            if (not input_command.empty())
            {
                ...
                try
                {
                    ...
                    {
                        if (not server::ServerInstance::getInstancePointer()->sendRawInput(input_command))
                        {
...
public:
    static auto getInstance() noexcept -> MainCLIThread &
    {
        static MainCLIThread instance;
        return instance;
    }

    auto run() -> void
    {
        this->thread_ = std::move(std::jthread(&MainCLIThread::mainCLILoop, this));
    }
