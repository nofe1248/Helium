...
private:
    ...
    {
        while (std::isblank(this->getCurrent()) and this->current_iterator_ <= this->raw_command_.cend())
        {
    ...
    auto getNext() noexcept -> typename StringType::value_type
    {
        ++this->current_iterator_;
    ...
    auto getCurrent() const noexcept -> typename StringType::value_type
    {
        if (this->current_iterator_ >= this->raw_command_.cend())
        {
            return -1;
    ...
    auto tryScanToken() -> std::expected<TokenType, LexError>
    {
        ...
        this->skipWhitespace();
        if (std::isdigit(this->getCurrent()))
        {
    ...
    auto tryScanNumber() -> std::expected<TokenType, LexError>
    {
        ...
        for (; this->current_iterator_ < this->raw_command_.cend() and not std::isblank(this->getCurrent());)
        {
            using namespace mpark::patterns;
            match(this->getNext())(
                pattern('0') = [&number_state_machine] { number_state_machine.process_event(Zero{'0'}); },
                pattern(anyof('1', '2', '3', '4', '5', '6', '7', '8', '9')) = [&number_state_machine, this] { number_state_machine.process_event(OneToNine{this->getCurrent()}); },
                pattern(anyof('e', 'E')) = [&number_state_machine] { number_state_machine.process_event(Exponent{'E'}); },
                pattern('+') = [&number_state_machine] { number_state_machine.process_event(Add{'+'}); }, pattern('-') = [&number_state_machine] { number_state_machine.process_event(Negative{'-'}); },
    ...
    auto tryScanQuotedString() -> std::expected<TokenType, LexError>
    {
        ...
        auto next_byte_in_range = [this, &add_to_result](std::vector<std::pair<char, char>> const &ranges) -> bool {
            add_to_result(this->getCurrent());
            for (auto const &[first, last] : ranges)
            {
                auto cur = this->getNext();
                if (cur >= first and cur <= last) [[likely]]
                {
                    add_to_result(cur);
                }
                else
                ...
                pattern('\\') =
                    [this, &add_to_result, &no_lex_error_occurred, &error_str] {
                        match(this->getNext())(
                            ...
                            pattern('/') = [&add_to_result] { add_to_result('/'); }, pattern('b') = [&add_to_result] { add_to_result('\b'); }, pattern('f') = [&add_to_result] { add_to_result('\f'); },
                            pattern('n') = [&add_to_result] { add_to_result('\n'); }, pattern('r') = [&add_to_result] { add_to_result('\r'); },
                            pattern('t') = [&add_to_result] { add_to_result('\t'); },
                            pattern('u') =
                                [this, &add_to_result, &no_lex_error_occurred, &error_str] {
                                    int codepoint = 0;
                    ...
                    [this, &add_to_result, &no_lex_error_occurred, &next_byte_in_range, &error_str](const char c) {
                        if (c >= 0x00 and c <= 0x1F)
                        {
                            ...
                            error_str = std::format("Invalid string: control character U+{:#06x} "
                                                    ...
                                                    static_cast<unsigned int>(this->getCurrent()), mapping[this->getCurrent()], static_cast<unsigned int>(this->getCurrent()), this->raw_command_);
                        }
